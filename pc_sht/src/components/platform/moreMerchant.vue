  ...
  <div v-if="show" class="myDiv" v-loading="Dataloading"></div>
			...
			<el-form ref="form" :inline="true" :model="form" label-width="100px">
			  ...
			  <div class="select">
			    <el-dropdown @command="handlerCommand" trigger="click" placement="bottom-start">
			      ...
			      <el-dropdown-menu slot="dropdown">
			        <el-dropdown-item :command="{value:'全部',flag:2,index:0}" :class="nodeNameIndex == 0 ? 'active':''">全部</el-dropdown-item>
			        <el-dropdown-item :command="{value:item.node_name,flag:2,index:index2 + 1}" :class="nodeNameIndex == index2 +1 ? 'active':''" v-for="(item,index2) in nodeNameList" :key="index2">{{item.node_name}}</el-dropdown-item>
			      </el-dropdown-menu>
			    </el-dropdown>
			  ...
			  <div class="select">
			    <el-dropdown @command="handlerCommand" trigger="click" placement="bottom-start">
			      ...
			      <el-dropdown-menu slot="dropdown">
			        <el-dropdown-item :command="{value:'全部',flag:3,index:0}" :class="modeIndex == 0 ? 'active':''">全部</el-dropdown-item>
			        <el-dropdown-item :command="{value:item.mode,flag:3,index:index3 + 1}" :class="modeIndex == index3 +1 ? 'active':''" v-for="(item,index3) in modeList" :key="index3">{{item.mode}}</el-dropdown-item>
			      </el-dropdown-menu>
			    </el-dropdown>
    ...
    <div class="mytable1" style="border-bottom: 3px solid #E8EBED;" v-loading="Dataloading1">
      ...
      <div class="draw-table1">
        <el-table :data="bizMonthStrandedList" border :header-cell-style="rowClass" :height="200" class="mytable-itme1">
          <el-table-column  prop="info_type" label="信息类型"> </el-table-column>
          <el-table-column  prop="num" label="数据量" ></el-table-column>
          <el-table-column  prop="active" label="商户活跃度" ></el-table-column>
    ...
    <div class="myecharts1" style="border-bottom: 3px solid #E8EBED;" v-loading="Dataloading2">
      ...
      <div id="echartsOne_merchant" class="myecharts" style="height: 300px;"></div>
    </div>
    <div class="myecharts2" style="border-bottom: 3px solid #E8EBED;" v-loading="Dataloading2">
      <p class="myecharts2-item2-title">商户活跃度</p>
      <p class="myecharts2-avgActive">{{avgActive}}</p>
      <p class="myp">平均</p>
      <div id="echartsTwo_merchant" class="myecharts" style="height: 250px;"></div>
    </div>
    <div class="myecharts3" style="border-bottom: 3px solid #E8EBED;" v-loading="Dataloading2">
      <p class="myecharts3-item3-title">价格指标分析</p>
      <div id="echartsThree_merchant" class="myecharts" style="height: 300px;"></div>
    </div>
    <div class="myecharts4" style="border-bottom: 3px solid #E8EBED;" v-loading="Dataloading2">
      <p class="myecharts4-item4-title">重量指标分析</p>
      <div id="echartsFour_merchant" class="myecharts" style="height: 300px;"></div>
    </div>
    <div class="mytable2" style="border-bottom: 3px solid #E8EBED;" v-loading="Dataloading3">
      <p class="mytable2-item2-title">运行指标汇总</p>
      <div class="draw-table2">
        <el-table :data="moreBusinessMonthDayList" border :header-cell-style="rowClass" :height="400">
          <el-table-column  prop="group_name" label="集团名称"> </el-table-column>
          <el-table-column width="172px"  prop="biz_id" label="商户编码" ></el-table-column>
          <el-table-column  prop="biz_name" label="商户名称" ></el-table-column>
          <el-table-column  prop="info_type" label="信息类型"></el-table-column>
          <el-table-column  prop="num" label="数据量"></el-table-column>
          <el-table-column  prop="days" label="报送天数"></el-table-column>
          <el-table-column  prop="price" label="价格"></el-table-column>
          <el-table-column  prop="weight" label="重量(公斤)"></el-table-column>
        </el-table>
      </div>
    </div>
    <div class="mytable3" style="border-bottom: 3px solid #E8EBED;" v-loading="Dataloading4">
      <p class="mytable3-item3-title">每日指标统计</p>
      <div class="draw-table3">
        <el-table :data="moreBusinessMonthStrandedDayList" border :header-cell-style="rowClass" :height="300">
          <el-table-column  prop="date" label="事件日期"> </el-table-column>
          <el-table-column width="172px"  prop="biz_id" label="商户编码" ></el-table-column>
          <el-table-column  prop="biz_name" label="商户名称" ></el-table-column>
          <el-table-column  prop="info_type" label="信息类型"></el-table-column>
          <el-table-column  prop="num" label="数据量(条)"></el-table-column>
...
<script>
import {getQueryNodeMonthType,getQueryNodeMonthStranded,getQueryNodeMonthStrandedNumAndWeight,getQueryMoreBusinessMonthDay,getQueryMoreBusinessMonthStrandedDay} from '../../js/platform/platform.js';
var preD = function (e) {
     e.preventDefault()
}
export default {
	...
	data() {
      ...
      modeIndex : 0,
      modeList : [],
      bizMonthStrandedList:[],
      moreBusinessMonthDayList:[],
      moreBusinessMonthStrandedDayList :[],
      avgActive : 0,
			form:{
			}
		}
    ...
    this.getQueryMoreBusinessMonthDayFun();
    ...
    getQueryNodeMonthStrandedFun(){
      ...
      if(this.selectThree == '企业名称'){
         node_name = ''
      }else{
        if(this.selectThree.indexOf(',')==-1){
          node_name = "'" + this.selectThree + "'";
        }else{
          let str = '';
      ...
      if(this.selectFour == '建设模式'){
         mode = ''
      }else{
        if(this.selectFour.indexOf(',')==-1){
          mode = "'" + this.selectFour + "'";
        }else{
          let str = '';
          ...
          mode = str.substr(0,str.length-1);
        }
      }
      let str = 'date=' + date + '&node_type=' + node_type + '&node_name=' + node_name + '&s_model=' + mode ;
      this.bizMonthStrandedList = [];
      getQueryNodeMonthStranded(str)
        ...
        //	  this.nodeMonthStrandedList = res.data.list
            res.data.list.forEach(ele=>{
              ...
              this.bizMonthStrandedList.push(map)
            })
    ...
    getQueryMoreBusinessMonthDayFun(){
      ...
      let str = 'date=' + date + '&node_type=' + node_type + '&node_name=' + node_name + '&s_model=' + mode ;
      this.moreBusinessMonthDayList = [];
      getQueryMoreBusinessMonthDay(str)
        .then(res => {
        	   // this.moreBusinessMonthDayList = res.data.list
              let list = res.data.list, arr = [];
               list.forEach((val1, ele1) => {
                   val1.group.forEach((val2, ele2) => {
                       val2.biz.forEach((val3, ele3) => {
                           if(ele3 == 0 && ele2 == 0){
                             val3.group_name = val1.group_name
                           }else{
                             val3.group_name = ''
                           }
                           if(ele3 == 0){
                             val3.biz_name = val2.biz_name
                             val3.biz_id = val2.biz_id
                           }else{
                             val3.biz_id = ''
                             val3.biz_name = ''
                           }
              ...
              this.moreBusinessMonthDayList = arr
    ...
    getQueryMoreBusinessMonthStrandedDayFun(){
      ...
      let str = 'date=' + date + '&node_type=' + node_type + '&node_name=' + node_name + '&s_model=' + mode ;
      this.moreBusinessMonthStrandedDayList = [];
      getQueryMoreBusinessMonthStrandedDay(str)
        .then(res => {
        	   // this.moreBusinessMonthDayList = res.data.list
              let list = res.data.list, arr = [];
               list.forEach((val1, ele1) => {
                   val1.time.forEach((val2, ele2) => {
                       val2.biz.forEach((val3, ele3) => {
                           if(ele3 == 0 && ele2 == 0){
                             val3.date = val1.date
                           }else{
              ...
              this.moreBusinessMonthStrandedDayList = arr
  ...
  /*  getData() {
    ...
    getQueryNodeMonthStrandedNumAndWeightFun1(lineValue1, arr,lineName,
            lineValue2,lineValue3,lineValue4,lineValue5,lineValue6,lineValue7,
            lineValue8){
            let echartsOne_merchant = this.$echarts.init(document.getElementById('echartsOne_merchant'));
            window.addEventListener('resize', function () {
              echartsOne_merchant.resize()
            })
            echartsOne_merchant.setOption({
               tooltip: {
                     trigger: 'axis'
                  },
               legend: {
               ...
               color:['#00aa00','#ffff00','#ff5500','#0000ff','#55aa00','#55aaff','#00ffff','#5500ff'],
               xAxis: {
                   type: 'category',
                   boundaryGap: false,
                   data: lineName
    ...
    getQueryNodeMonthStrandedNumAndWeightFun2(lineDate,lineActive){
            let echartsTwo_merchant = this.$echarts.init(document.getElementById('echartsTwo_merchant'));
            window.addEventListener('resize', function () {
              echartsTwo_merchant.resize()
            })
            echartsTwo_merchant.setOption({
              tooltip: {
                trigger: 'axis'
              },
              xAxis: {
    ...
    getQueryNodeMonthStrandedNumAndWeightFun3(linePrice1, arr,lineName,
            ...
            let echartsThree_merchant = this.$echarts.init(document.getElementById('echartsThree_merchant'));
            window.addEventListener('resize', function () {
              echartsThree_merchant.resize()
            })
            echartsThree_merchant.setOption({
               tooltip: {
                     trigger: 'axis'
                  },
               legend: {
    ...
    getQueryNodeMonthStrandedNumAndWeightFun4(lineChar1, arr,lineName,
            ...
            let echartsFour_merchant = this.$echarts.init(document.getElementById('echartsFour_merchant'));
            window.addEventListener('resize', function () {
              echartsFour_merchant.resize()
            })
            echartsFour_merchant.setOption({
               tooltip: {
                     trigger: 'axis'
                  },
               legend: {
