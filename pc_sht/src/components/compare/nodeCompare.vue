        ...
        <div class="table" v-loading="loading">
            <div class="title">
                ...
                <div>
                    ...
                    <el-button plain @click="allEditFun">{{editAllBtn}}</el-button>
                </div>
            </div>
            <el-table :data="tableData" :header-cell-style="rowClass" :style="editAllBtn == '批量修改' ? {'display': 'block'} : {'display': 'none'}">
                <el-table-column prop="before_code" label="节点编码"></el-table-column>
                <el-table-column prop="before_name" label="节点名称"></el-table-column>
                <el-table-column prop="userdefine_code" label="自定义节点编码"></el-table-column>
                <el-table-column prop="userdefine_name" label="自定义节点名称"></el-table-column>
            ...
            <el-table :data="tableData" :header-cell-style="rowClass" :style="editAllBtn == '批量修改' ? {'display': 'none'} : {'display': 'block'}">
                <el-table-column prop="before_code" label="节点编码">
                    <template slot-scope="scope">
                        <el-input v-model="scope.row.before_code" clearable @input="inputFun(scope.row)"></el-input>
                    </template>
                </el-table-column>
                <el-table-column prop="before_name" label="节点名称">
                    <template slot-scope="scope">
...
export default {
    ...
    methods: {
        ...
        downloadFun(){
            ...
            DownloadNodeUserdefine( params, {})
                .then((res) => {
                    ...
                    aLink.href = url;
                    aLink.setAttribute("download", `节点对照` + time);
        ...
        fileFun(event){
            ...
            formData.append('userdefine_type', 2);  
        ...
        addFun(){
            ...
            this.$router.push({name: 'NewNodeCompare'})
        },
        editFun(ele){
            localStorage.setItem('searchMsg2', JSON.stringify(this.form))
            this.$router.push({name: 'EditNodeCompare', params: ele})
        },
        deleteFun(ele, index){
            let str = 'id=' + ele.id
