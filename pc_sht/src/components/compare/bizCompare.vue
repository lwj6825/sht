    ...
    <div class="content bizCompare">
        ...
        <div class="table" v-loading="loading">
            ...
            <el-table :data="tableData" :header-cell-style="rowClass" :style="editAllBtn == '批量修改' ? {'display': 'block'} : {'display': 'none'}">
                <el-table-column prop="before_code" label="商户编码"></el-table-column>
                <el-table-column prop="before_name" label="商户名称"></el-table-column>
                <el-table-column prop="userdefine_code" label="自定义商户编码"></el-table-column>
                <el-table-column prop="userdefine_name" label="自定义商户名称"></el-table-column>
            ...
            <el-table :data="tableData" :header-cell-style="rowClass" :style="editAllBtn == '批量修改' ? {'display': 'none'} : {'display': 'block'}">
                <el-table-column prop="before_code" label="商户编码">
                    <template slot-scope="scope">
                        <el-input v-model="scope.row.before_code" clearable @input="inputFun(scope.row)"></el-input>
                    </template>
                </el-table-column>
                <el-table-column prop="before_name" label="商户名称">
                    <template slot-scope="scope">
...
export default {
    ...
    methods: {
        ...
        downloadFun(){
            ...
            DownloadNodeUserdefine( params, {})
                .then((res) => {
                    ...
                    aLink.href = url;
                    aLink.setAttribute("download", `商户对照` + time);
        ...
        addFun(){
            ...
            this.$router.push({name: 'NewBizCompare'})
        },
        editFun(ele){
            localStorage.setItem('searchMsg2', JSON.stringify(this.form))
            this.$router.push({name: 'EditBizCompare', params: ele})
        },
        deleteFun(ele, index){
            let str = 'id=' + ele.id
        ...
        fileFun(event){
            ...
            formData.append('userdefine_type', 1);  
