        ...
        <div class="table">
            <div class="title">
                ...
                <div>
                    ...
                    <el-button plain @click="isShowFun(1)">生活必需品导入</el-button>
                    <el-button plain @click="isShowFun(2)">导出</el-button>
                </div>
                </div>
            </div>
            <div class="tables" >
                <el-table :data="tableData" :header-cell-style="rowClass" v-loading="Dataloading">
                    ...
                    <el-table-column prop="num" label="上报商品数量">
                      <template slot-scope="scope">
                        {{'已上报'+ scope.row.num + '种商品，包含' + scope.row.bxp_num + '种生活必需品'}}
                      </template>
                    </el-table-column>
            ...
            <!--新增-->
            <div class="passwrd" v-if="isEdits" v-loading="save_loading">
                <div class="text">
                    <div class="box-title">
                    ...
                    <div class="table">
                        <el-table :data="tableData2" :header-cell-style="rowClass" height="360" v-loading="loading">
                            ...
                            <el-table-column prop="goods_unit" label="规格" width="140">
                                <template slot-scope="scope">
                    ...
                    <div class="btn">
                        ...
                        <span>共{{tableData2.length}}种商品，已填报{{retail_count}}种</span>
                        <el-button style="margin-left: 140px"@click="closeFun">取消</el-button>
            ...
            <!--智能录入 :style="isAgain ? {'margin-left': '230px'} : {'margin-left': '140px'}"-->
            ...
            <div class="passwrd" v-if="isView">
                <div class="text">
                    ...
                    <div class="btn">
                        ...
                        <el-button @click="closeFun4" style="margin-left: 390px;">关闭</el-button>
                    </div>
                </div>
            </div>
            <!--查看商品详细信息-->
...
import {getQueryNodeInfoIndex,getQueryIndex,getQueryGoodsForBiz,getInsert,getQueryRegion,AutoIdentity,getInsertList,getQueryGoodsForNode,
    getQueryRegionForGoodsPrice,getQueryGoodsIndex,getQueryIndexDate,getQueryNodeGoodsBiz, getAutoIdentity} from '../../js/quotation/quotation.js'
...
import {DownloadPriceLife, GetAllNodePage} from '../../js/retail/retail.js';
...
export default {
    ...
    methods: {
        getGetAllNodePage(){
            ...
            GetAllNodePage(obj)
                .then(res => {
                    this.userId = res.data.dataList[0].USERID
                })
                .catch(res => {
        ...
        downloadFun(){
            ...
            DownloadPriceLife( params, {})
                .then((res) => {
                    ...
                    aLink.href = url;
                    aLink.setAttribute("download", (node_name + '报价' + in_date));
        ...
        fileFun(event){
            ...
            formData.append('priceLife', this.file);  
            formData.append('node_id', this.node_id); 
            formData.append('in_date', this.in_date); 
            formData.append('unit_type', 1); 
            let config = {
            ...
            if(val.id==null){
                if(this.addGood.length == 0){
                    // if(val.price){
                        ...
                        this.addGood.push(goodobj)
                    // }
                }else{
                    this.addGood.forEach((ele,index) => {
                        if(val.goods_id == ele.goods_id){
                            this.addGood.splice(index,1)
                        }
                    })
                    goodobj = {
                        node_id: val.node_id,
                    ...
                    .then(res => {
                        ...
                        res.data.list.forEach(val => {
                            if(this.isSearch == false){
                                ...
                                if(val.price && this.viewGood.length == 0 && this.addGood.length == 0){
                                    if(val.goods_unit == '斤'){
                                        val.price = val.price/2
                                    }
                                }
        ...
        submitForm(ele){
                    ...
                    this.selectGood.forEach(val => {
                        if(val.price){
                            ...
                            goodobj = {
                                ...
                                node_name: val.node_name,
                                shop_booth_id: this.merchant ? this.merchant : '',
                                biz_id: this.biz_id ? this.biz_id : '',
                                biz_name: this.biz_name ? this.biz_name : '',
                                in_date: this.in_date,
                                goods_id: val.goods_id,
                                goods_code: val.goods_code,
                                goods_name: val.goods_name,
            ...
            if(arr.length > 0 || this.viewGood.length > 0 || this.addGood.length > 0){
                if(this.viewGood.length > 0){
                ...
                if(this.addGood.length > 0){
                    let addGood = []; // 只取有价格数据
                    this.addGood.forEach((val,index) => {
                        if(val.price){
                            addGood.push(val)
                        }
                    })
                    let addobj = {};
                    arr.forEach(val2 => {
                        addGood.forEach((val,index) => {
                            if(val.goods_id == val2.goods_id){
                                addGood.splice(index,1)
                            }
                        })
                    })
                    arr = arr.concat(addGood)
                }
                if(this.unit == 1){
                    arr.forEach(val => {
                        if(val.price){
                ...
                console.log(saveArr)
                let obj =  JSON.stringify(saveArr)
                getInsertList(obj)
                    .then(res => {
        ...
        againFun(ele){
            this.chooseMer = true;
            this.tbqyUserId = this.userId;
            this.tbqy = this.node_id;
        ...
        viewFun(ele){
            ...
            this.tbqyView = this.node_name;
            this.viewNodeId = this.node_id;
            this.page2 = 1;
            this.loading2 = true;
        ...
        merchantFun(){
          ...
          this.$router.push({name: 'ViewRetail',params: obj})
        },
        getDataFun(){
            let params = {
