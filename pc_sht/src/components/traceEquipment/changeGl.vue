<template>
    <div class="content changeGl" @click="closeFun($event)">
        <div class="searchs" ref="searchs">
            <div class="search">
                ...
                <el-form ref="form" :inline="true" :model="form" label-width="80px" :style="show ? {display: 'block'} : {display: 'none'}">
                    ...
                    <el-form-item label="变更日期" style="width: 380px;" >
                    ...
                    <el-form-item label="变更内容">
                        <el-select v-model="form.content" filterable clearable placeholder="请选择">
                            <el-option v-for="(item,index) in contentArr" :key="index" :label="item"
                            :value="item">
                            </el-option>
                        </el-select>
                    </el-form-item>
        ...
        <div class="table">
            ...
            <div class="tables" >
                <el-table :data="tableData" :header-cell-style="rowClass">
                    ...
                    <el-table-column prop="bar_code" label="条码号" width="100"> </el-table-column>
                    <el-table-column prop="c_item" label="变更内容" width="80"></el-table-column>
                    ...
                    <el-table-column prop="c_time" label="操作时间" width="160"> </el-table-column>
                    <el-table-column prop="node_name" label="所属市场" > </el-table-column>
...
import {QueryAssetsUser,QueryAssetsConf,QueryAssetsType,QueryNodeBase,QueryBusiness,QueryAssetsBase,DownloadChangeLog,
    AssetsAdd,AssetsUpdate,ImportAssets,ImportAssetsUpdate,exportExcel,QueryChangeLogList,QueryChangeItem
...
export default {
    ...
    mounted() {
        ...
        this.getQueryChangeItemFun()
        this.userId = localStorage.getItem('userId')
    },
    methods: {
        // 变更内容
        getQueryChangeItemFun(){
            QueryChangeItem('')
                .then(res => {
                    this.contentArr = res.data.list
                })
                .catch(res => {
                    console.log(res);
                })
        ...
        getQueryAssetsBase(){
            ...
            let params = {
                ...
                c_creater: this.form.name,
                cols: this.cols,
                page: this.page,
            }
            QueryChangeLogList(params)
                .then(res => {
                    res.data.change_log_list.forEach(ele => {
                        if(ele.c_item == '附件'){
                            ele.c_before = ele.c_before.split(',')
                            ele.c_after = ele.c_after.split(',')
                        }else{
                            return
                        }
                    })
                    this.tableData = res.data.change_log_list
                    this.num = res.data.change_log.total
                    loading.close();
                })
                .catch((res) => {
        ...
        getDownloadAssetsBase(){
            ...
            //         window.location.href = objectUrl;
                    ...
                    aLink.href = url;
                    aLink.setAttribute("download", `变更` + time);
        ...
        viewFun(ele){
            this.$router.push({name: 'ViewChangeGl',params: {param: ele}})
        },
        newFun(){
